...
const SoldierCardPage: React.FC<SoldierCardPageProps> = ({ accessToken }) => {
    ...
    const { data, isLoading: isLoadingSoldier, error } = useGoogleSheetData(
    ...
    const isLoading = isLoadingSoldier || isLoadingHeaders;
    ...
    const primaryKeys = ['שם מלא', 'מספר אישי', 'פלאפון', 'זמן חתימה'];
        ...
        : (
            <div className="p-6 max-w-4xl mx-auto" dir="rtl">
                ...
                <div className="flex justify-between text-sm mb-4">
                    ...
                    <div>שם מלא: {row['שם מלא'] || '-'}</div>
                </div>

                <div className="flex justify-between text-sm mb-4">
                    <div>תאריך חתימה: {row['זמן חתימה'] || '-'}</div>
                    <div>מספר אישי: {row['מספר אישי'] || '-'}</div>
                </div>
